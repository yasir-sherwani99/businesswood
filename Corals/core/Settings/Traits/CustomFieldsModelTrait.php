...
trait CustomFieldsModelTrait
{
    ...
    public function customFieldSettings($status = 'active')
    {
        ...
        $customFieldSettings = Cache::remember($cache_key, config('corals.cache_ttl'), function () use ($className, $status) {
    ...
    public function customFieldNames()
    {
        ...
        $cache_key = \Str::slug($className) . '_cf_name';

        $customFieldSettings = Cache::remember($cache_key, config('corals.cache_ttl'), function () use ($className) {
    ...
    public function save(array $options = array())
    {
        ...
        foreach ($this->customFieldNames() as $name) {
            ...
            if ($customFieldModel instanceof CustomField) {
                ...
                if (\Arr::has($this->customAttributes, $name)) {
    ...
    public function fillCustomAttributes(array &$attributes)
    {
        foreach ($this->customFieldNames() as $name) {
            if (\Arr::has($attributes, $name)) {
                $this->customAttributes[$name] = \Arr::pull($attributes, $name, null);
            }
        }
    }
    ...
    {
        return \Arr::get($this->relations, $key, null);
    }
    ...
    {
        $model = $this->getCustom($fieldName);
     ...
     * @return mixed
     */
    public function getCustom($fieldName)
    {
    ...
    public function presentCustomFieldsValues($labelTag = 'span', $valueTag = 'b', $spacer = ': ', $get = ['except' => [], 'only' => []], $colClass = 'col-md-6')
    {
        ...
        foreach ($customFieldNames as $name) {
            ...
            $fields [] = HtmlElement($labelTag, $customFieldSetting->label) . $spacer
                . HtmlElement($valueTag, $this->getCustom($name));
        }
