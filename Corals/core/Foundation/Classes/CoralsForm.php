...
class CoralsForm
{
    ...
    public function input($key, $label = '', $required = false, $value = null, $attributes = [], $type)
    {
        ...
        if (in_array($type, $this->selectTypes)) {
            ...
            $input .= $this->number($key . "['from']", '', $required, $value, $attributes);
            $input .= '<div class="input-group-addon">to</div>';
            $input .= $this->number($key . "['to']", '', $required, $value, $attributes);
            $input .= '</div>';
        } else {
    ...
    public function select($key, $label = '', $options = [], $required = false, $value = null, $attributes = [], $type = 'select')
    {
        ...
        if ($type == 'select') {
            ...
            $attributes['data-placeholder'] = \Arr::get($attributes, 'data-placeholder', $placeholder);
            ...
            if (!\Arr::get($attributes, 'multiple', false)) {
                ...
                if ($type != 'select2-tree') {
                    ...
                    $options = array_merge([['id' => null, 'text' => $attributes['data-placeholder'], 'selected' => true]], $options);
                }
            }
        }
    ...
    public function customFields($model, $fieldClass = 'col-md-4')
    {
        ...
        foreach ($customFields as $field) {
            $value = $model->exists ? $model->getCustom($field->name) : $field->default_value;
